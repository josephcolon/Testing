...
## Play modes
- **Free Drive** — a deep, open, no-fail driving world. The car rides over
  **rolling hills** (tilting to the slope) and travels through changing
  **biomes** (grassland → desert → snow → city → beach), each with its own sky,
  ground, hills, animals and landmarks. Drive into **coins/balloons** to collect
  them (they feed the Prize Machine), hit a **ramp** to launch a **jump**, watch
  a **day/night cycle** drift by, and **honk** to make the animals call back
  ("Moo!", "Woof!"). A coin counter + **distance odometer** track the trip.
  Endless in both directions, no obstacles. Two hold-to-drive arrows (◀ ▶), a
  honk button, and a garage (🔧) to change car type + color — saved per child.
