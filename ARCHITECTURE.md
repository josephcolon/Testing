...
**`src/content/lines.js`** — catalog of every user-facing spoken/prompt line: `LINES[id] = {text, file:'vo/<id>.mp3'}`, plus `line(id)`→text, `lineFile(id)`, and `GROUPS` (categorised, source for `recording-script.md`). Atom IDs (`c_*`, `s_*`, `l_a..l_z`, `n_0..n_10`, nouns, instruction stems, cheers, `try_again`, `reward`) match the voice-token vocabulary so mini-game `speechTokens` already reference catalog IDs; full lines use readable IDs. `themes.js praise` arrays hold catalog IDs, resolved by engine via `line()`. mp3s are NOT played yet — this is content/source-of-truth only. Call sites reference IDs via `line('id')` (screens, engine fallbacks, drive `SOUND` map, static-prompt activities); dynamic prompt display text stays assembled from data, its speech via `speechTokens` = catalog atom IDs. `linematch.js` imports it as `vo` (local `line()` SVG helper collides). Independent of `voice.js` VOCAB (parent-record feature).
